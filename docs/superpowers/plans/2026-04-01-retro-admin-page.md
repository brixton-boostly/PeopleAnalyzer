# Retro Admin Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a dedicated Retro page inside each admin cycle with Setup tab (edit questions, select employees, launch) and Responses tab (track submissions, view responses, copy links).

**Architecture:** New server page at `/admin/[cycleId]/retro` fetches cycle + employees + retros and passes them to `RetroAdminView`, a client component that manages tab state. The existing `/api/retro/send` route is extended to accept a `participantIds` array so only selected employees receive the retro. Admin sidebar gets a 4th nav item linking to the new page.

**Tech Stack:** Next.js 16 App Router, TypeScript strict, Tailwind CSS, Supabase, `next-auth` v5 beta
...
|------|--------|---------------|
| `lib/types.ts` | Modify | Add `retro_participant_ids` to `ReviewCycle` |
| `app/api/retro/send/route.ts` | Modify | Accept + persist `participantIds`; filter send list |
| `components/admin/AdminShell.tsx` | Modify | Add Retro nav item |
| `components/admin/RetroAdminView.tsx` | Create | Setup + Responses tabs UI |
| `app/(admin)/admin/[cycleId]/retro/page.tsx` | Create | Server page — fetch + render |
...
> **Before writing code:** Run this SQL in the Supabase SQL editor (Dashboard → SQL Editor):
> ```sql
> ALTER TABLE review_cycles ADD COLUMN IF NOT EXISTS retro_participant_ids JSONB;
> ```
> Verify it succeeded by running `SELECT retro_participant_ids FROM review_cycles LIMIT 1;` — it should return a column (null values are fine).

- [ ] **Step 1: Add `retro_participant_ids` to the `ReviewCycle` interface**

Open `lib/types.ts`. The current `ReviewCycle` interface (lines 28–36) is:
```ts
...
```bash
cd /Users/brixton/Desktop/BRIXTON/boostly-people-analyzer && npx tsc --noEmit
```

Expected: no errors (existing code doesn't use `retro_participant_ids` yet so no breakage).

- [ ] **Step 3: Commit**

```bash
git add lib/types.ts
git commit -m "feat: add retro_participant_ids to ReviewCycle type"
```

---

## Task 2: Extend `/api/retro/send` to accept participantIds

**Files:**
- Modify: `app/api/retro/send/route.ts`

The current file sends to ALL employees in the cycle. We need it to:
1. Accept optional `participantIds: string[]` in the request body
2. Persist them to `review_cycles.retro_participant_ids`
3. Filter the send list to only those IDs when provided

- [ ] **Step 1: Replace the route implementation**

Full replacement of `app/api/retro/send/route.ts`:
...
```bash
git add app/api/retro/send/route.ts
git commit -m "feat: retro/send accepts participantIds + persists to cycle"
```

---

## Task 3: Add Retro nav item to AdminShell

**Files:**
- Modify: `components/admin/AdminShell.tsx`

The current "Current Cycle" section (around line 52–58) renders Overview, Assignments, Results. Add Retro after Results.

- [ ] **Step 1: Add the Retro nav item**

In `components/admin/AdminShell.tsx`, find this block:
```tsx
...
git commit -m "feat: add Retro nav item to admin sidebar"
```

---

## Task 4: Build `RetroAdminView` component

**Files:**
- Create: `components/admin/RetroAdminView.tsx`

This is the main UI component. It receives all data from the server page as props and manages local tab + interaction state.

Props shape used throughout this task:
```ts
interface Employee {
  id: string
  full_name: string
  ...
  async function launchRetro() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLaunchError(body.error ?? 'Launch failed')
      }
    } catch {
...
```bash
git add components/admin/RetroAdminView.tsx
git commit -m "feat: RetroAdminView component with Setup + Responses tabs"
```

---

## Task 5: Create the retro admin server page

**Files:**
- Create: `app/(admin)/admin/[cycleId]/retro/page.tsx`

This is a Next.js 16 server page. Key patterns from the codebase:
- `params` is `Promise<{ cycleId: string }>` — must be awaited
- Auth check: `const session = await auth()` → redirect if not admin
- Supabase client: `const supabase = createClient()`
- Redirect: `redirect('/admin')` if cycle not found

The page needs to:
1. Fetch cycle fields: `name, retro_status, retro_questions, retro_participant_ids`
2. Fetch all unique employees in the cycle (via `manager_assignments` JOIN `direct_reports` and `users!manager_id`)
3. Fetch all retros for this cycle
...
```bash
cd /Users/brixton/Desktop/BRIXTON/boostly-people-analyzer && npm run build 2>&1 | tail -20
```

Expected: `✓ Compiled successfully` with no type errors. Build may warn about static/dynamic pages — that's fine.

- [ ] **Step 4: Commit**

```bash
git add app/\(admin\)/admin/\[cycleId\]/retro/page.tsx
git commit -m "feat: retro admin page — Setup + Responses tabs with employee selection"
```
...
After all tasks are committed, verify end-to-end in the running app:

1. **Nav item visible** — Go to any cycle in `/admin/[cycleId]`. Confirm "🔁 Retro" appears in the left sidebar below Results.
2. **Setup tab (pre-launch)** — Click Retro. Confirm:
   - 3 default questions shown as editable textareas
   - × buttons on each question (disabled when only 1 remains)
   - "+ Add question" adds a blank row
   - All employees listed with checkboxes checked by default
   - Select All / Clear All work
   - Launch button shows correct count and is disabled when 0 selected
3. **Launch** — Click Launch. Confirm page refreshes and switches to Responses tab automatically (because `retro_status` flips to `active` → `tab` initializes to `responses`).
4. **Responses tab (post-launch)** — Confirm:
   - Stat cards show correct Sent / Submitted / Pending counts
   - Only the selected participants are listed (not the full roster)
   - "Copy Link" copies a URL to clipboard for pending employees
   - "View" opens the slide-over with full responses for submitted employees
   - Setup tab shows "locked" banner
